...
matchRouter.get("/", async (req, res) => {
    try {
        const matches = await Match.find().populate("club_A club_B user footballField bookingId orderId");
...
matchRouter.get("/:id", async (req, res) => {
    try {
        const match = await Match.findById(req.params.id).populate("club_A club_B user footballField bookingId orderId");
...
matchRouter.get("/footballField/:id", async (req, res) => {
    try {
        const match = await Match.find({ footballField: req.params.id }).populate("club_A club_B user footballField bookingId orderId");
        res.status(200).json(match);
    } catch (error) {
        res.status(500).json({ message: error.message });
    }
